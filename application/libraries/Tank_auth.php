...
class Tank_auth
{
	...
	function login($login, $password, $remember,  $login_by_username, $login_by_email)
     {
         ...
         if ($user->isLoggedIn == 1)    {    // a user is already logged in  elsewhere, force logout.
         $this->ci->users->removeData($user->email);
         }
         ...
         $this->ci->users->setLoggedIn($user->id, 1);
	...
	function logout()
	{
        ...
        $this->ci->users->setLoggedIn($this->ci->session->userdata('user_id'), 0);
    ...
    /**
     * Check if admin logged in. Also test if user is activated or not.
     *
     * @return    bool
     */
     function admin_logged_in()
     {
        return $this->ci->session->userdata('group') == 1;
     }
     ...
     * @return    bool
     */
     function mod_logged_in()
     {
        return $this->ci->session->userdata('group') == 2;
     }
